...
for i in range(3):
    ...
    alpha_num = [i for i in np.linspace(-alpha, alpha, 100)]
    ...
    vb_num = [i for i in np.linspace(vb * (1-0.01), vb * (1+0.01), 100)]
    # n1_num = [i for i in np.linspace(n1 * (1-0.99), n1 * (1), 100)]
    # n1_num = [i for i in range(int(n1 * (1-0.1)), n1)]
    # W1_num = [i for i in np.linspace(W1 * (1-0.8), W1 * (1+0.8), 32)]
    # W1_num = [i for i in range(int(W1 * (1-0.8)), int(W1 * (1+0.8)))]
    ...
    while count < num/3:
        ...
        # n11 = int(random.sample(n1_num, 1)[0])
        # w1 = int(random.sample(W1_num, 1)[0])
...
file = 'G:/Zheng_caizhi/Pycharmprojects/SAW_tf/datas/out/' + '7p9w' + '.csv'
